...
def run_mcmc_chains(dfparams, x, y, dy, save_complete_chains=False, savefile=None, niter=1000):
    ...
    # Spread factor for broader sampling:
    spread_factor = 0.1 * np.sqrt(ndim)

    # Generate initial walker positions with spread around p0, clipped to parameter limits
    theta0 = np.array([
         np.clip(p0 + spread_factor * np.random.randn(ndim), min_values, max_values)
