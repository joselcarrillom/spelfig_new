...
def initial_dataframe(emlines_dict, filtered_linelist, continuum_pars=None):
    ...
    for line in filtered_linelist:
        ...
        max_loc = line['max_loc']+Ncomp*line['sigma']
        min_loc = line['min_loc']-Ncomp*line['sigma']
        for j, component in enumerate(components):
            if component == 'Voigt':
                params_i = [line['wavelength'], line['max_flux']/Ncomp, line['sigma'],
                            1.11*line['sigma']]
                max_i = [max_loc, line['max_flux'], line['max_sd'], 1.11*line['max_sd']]
                min_i = [min_loc, 0., line['min_sd'], 1.11*line['min_sd']]
            elif component == 'Gaussian':
                params_i = [line['wavelength'], line['max_flux']/Ncomp, line['sigma']]
                max_i = [max_loc, line['max_flux'], line['max_sd']]
                min_i = [min_loc, 0., line['min_sd']]
            elif component == 'Lorentzian':
                params_i = [line['wavelength'], line['max_flux']/Ncomp, 1.11*line['sigma']]
                max_i = [max_loc, line['max_flux'], 1.11*line['max_sd']]
                min_i = [min_loc, 0., 1.11*line['min_sd']]
            elif component == 'Asymmetric Gaussian':
                params_i = [line['wavelength'], line['max_flux']/Ncomp, 0.1, 0.9*line['sigma']]
                max_i = [max_loc, line['max_flux'], 1.0, line['max_sd']]
                min_i = [min_loc, 0., 0., 1.11*line['min_sd']]
...
def spl_plot(x, y, dy, dfparams, x_zoom=None, y_zoom=None, goodness_marks=None):
    ...
    if goodness_marks:
        chi2 = goodness_marks['chi squared']
        BIC = goodness_marks['BIC']
        ax1.text(0.05, 0.85, r'$\chi^2$: {:.4f}'.format(chi2), transform=ax1.transAxes, fontsize=12,
                 color='gray', alpha=0.8)
        ax1.text(0.05, 0.78, f'BIC: {BIC:.4f}', transform=ax1.transAxes, fontsize=12,
                 color='gray', alpha=0.8)
